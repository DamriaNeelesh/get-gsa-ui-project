...
export const HomeClient = () => {
  ...
  const [appliedFilters, setAppliedFilters] = useState<FilterState>(() => computeInitialFilters());
  ...
  const setAsideOptions = useMemo(() => {
    const unique = Array.from(new Set(applications.flatMap((app) => app.setAside)));
    return unique.sort().map((value) => ({ value, label: value }));
  }, [applications]);
  ...
  const highlightTerms = useMemo(() => {
    const base = new Set(appliedFilters.keywords.map((keyword) => keyword.toLowerCase()));
    if (debouncedQuery) {
  ...
  const handleMarkSubmitted = (id: string) => {
    setApplications((prev) =>
      prev.map((app) => (app.id === id ? { ...app, status: "Submitted", percentComplete: 100 } : app)),
    );
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 pt-8 sm:px-6 lg:flex-row lg:gap-8 lg:px-10">
        <aside className="flex w-full flex-col gap-5 rounded-2xl border border-neutral-200 bg-white p-6 shadow-subtle lg:sticky lg:top-8 lg:h-fit lg:w-80">
          ...
          <div className="mt-4 flex flex-col gap-3">
            ...
            {filtersChanged ? (
              <p className="text-xs text-neutral-500">Unsaved filter changes will apply once you press Apply.</p>
            ) : null}
          </div>
        </aside>
